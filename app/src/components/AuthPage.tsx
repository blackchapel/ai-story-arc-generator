...
export const AuthPage = memo<AuthPageProps>(({ onSuccess, onBack: onBackProp, redirectAfter }) => {
  ...
  const handleSendOtp = useCallback(async () => {
    ...
    try {
      ...
      setErrorWithShake(err instanceof Error ? err.message : "Failed to send code. Please try again.");
    } finally {
      setLoading(false);
    }
  ...
  const handleVerifyOtp = useCallback(async (code: string) => {
    if (code.length !== 6) { setErrorWithShake("Please enter the 6-digit code."); return; }
    setLoading(true);
    setError("");
    try {
      ...
      setErrorWithShake(err instanceof Error ? err.message : "Invalid code. Please try again.");
  ...
  // ── Auto-submit ───────────────────────────────────────────────────────────
  const scheduleAutoSubmit = useCallback((filled: string[]) => {
    if (autoSubmitTimer.current) clearTimeout(autoSubmitTimer.current);
    if (filled.every((d) => d !== "")) {
      autoSubmitTimer.current = setTimeout(() => handleVerifyOtp(filled.join("")), 300);
    }
  }, [handleVerifyOtp]);
  ...
  const handleDigitKeyDown = useCallback(
    (index: number, e: React.KeyboardEvent<HTMLInputElement>) => {
      if (e.key === "Backspace") {
        if (digits[index] !== "") {
          const next = [...digits]; next[index] = ""; setDigits(next); scheduleAutoSubmit(next);
        } else if (index > 0) {
          const next = [...digits]; next[index - 1] = ""; setDigits(next);
          digitRefs.current[index - 1]?.focus(); scheduleAutoSubmit(next);
        }
        e.preventDefault();
  ...
  return (
    ...
    >
      ...
      <div
        ...
        // @ts-expect-error -- inert is valid HTML not yet typed
        inert={step !== "email" ? "" : undefined}
        style={{
      ...
      >
        ...
        >
          <input
            ...
            onBlur={(e) => {
              (e.currentTarget as HTMLInputElement).style.borderColor = error && step === "email" ? "#EF4444" : "#EBEBEB";
        ...
        <div
          ...
          style={{ bottom: btnBottom, transition: "bottom 0.28s cubic-bezier(0.4,0,0.2,1)" }}
        >
          <button
          ...
          >
            {loading ? (<><Spinner /><span>Sending code…</span></>) : "Login / Sign up"}
          </button>
        </div>
      </div>
      ...
      >
        ...
        <div className="w-full max-w-[360px] px-6">
          <h1 className="text-[26px] font-bold leading-tight text-[#0C0C0C]">Check your email</h1>
          <p className="mt-1.5 text-[15px] leading-relaxed text-[#8C8C8C]">
        ...
        >
          <div className="flex justify-between gap-2" role="group" aria-label="Verification code">
            {digits.map((digit, i) => (
              <input
                key={i}
                ref={(el) => { if (el) digitRefs.current[i] = el; }}
                type="text"
                inputMode="numeric"
                maxLength={1}
                value={digit}
        ...
        <div className="mt-5 flex items-center gap-1">
          <span className="text-[13px] text-[#8C8C8C]">Didn&apos;t get it?</span>
          <button
